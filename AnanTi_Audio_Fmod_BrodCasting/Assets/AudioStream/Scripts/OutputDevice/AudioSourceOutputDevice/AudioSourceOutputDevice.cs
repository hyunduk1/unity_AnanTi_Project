...
namespace AudioStream
{
    ...
    public class AudioSourceOutputDevice : MonoBehaviour
    {
        ...
        #region Editor
        [Header("[Setup]")]
        [Tooltip("Turn on/off logging to the Console. Errors are always printed.")]  //콘솔에 대한 로깅을 켜거나 끕니다. 오류는 항상 인쇄됩니다.
        ...
        //시스템에 있는 사용 가능한 오디오 출력 장치를 지정하고 0과 (오디오 출력 장치 수) - 1 사이의 정수를 전달합니다(0은 기본 출력).\r\n이 ID는 런타임 시 실제로 사용된 출력 ID - 이 'RuntimeOutputDriverID는 런타임 시 출력을 나타냅니다.\r\nRuntimeOutputDriverID는 AudioStreamDevicesChangedNotify 구성 요소가 있는 게임 개체가 장면에 있을 때 런타임 시 장치 추가/제거를 반영합니다. 데모 장면에서 FMOD_SystemW.AvailableOutputs / AudioStreamDevicesChangedNotify의 알림 이벤트를 참조하십시오.
        //[SerializeField]
        ...
        //런타임 시 출력 #outputDriverID를 사용할 수 없는 경우 이 구성 요소가 실제로 사운드를 재생하는 출력이 무엇인지 나타냅니다.
        [SerializeField]
        [ReadOnly]
        int runtimeOutputDriverID ;
        ...
        protected virtual void StartFMODSoundAndSystem(int onOutputDriverID, bool rememberOutput)
        {
            ...
            Debug.LogFormat(@"Updating {0} ", this.outputDriverID); //newOutputDriverID);
